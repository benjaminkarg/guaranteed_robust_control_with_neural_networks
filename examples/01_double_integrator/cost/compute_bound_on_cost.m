...
%% Params
u_lb = [-1.0]; % lower bound of control input
u_ub = [ 1.0]; % upper bound of control input
...
%% Load analytic approximation of minimum RPI
load('./../data/iterative_min_RPI_exacter.mat');
min_RPI_analytic_exacter = Polyhedron(H{end, 1}, h{end, 1});
...
%% compute worst-case cost for milp - based min RPI
max_cost_milp_exacter = max_stage_cost(network, min_RPI_analytic_exacter, Q, R, A, B, E);
...
%% save
save('./../data/worst_case_cost_min_RPI.mat', 'max_cost_milp', 'max_cost_data', 'max_cost_milp_exacter');
