...
# plot cost bound
max_cost = sio.loadmat('./../data/worst_case_cost_min_RPI.mat')['max_cost_milp']
ub = np.ones((avg_cost[0].shape[0], 1)) * max_cost;
ax.plot(ub, c = 'C1', lw = 3.0, ls = '-.', label = r'$\bar{\ell}$')
ax.set_xlim([0, 25])
ax.legend()
fig.tight_layout()
plt.savefig("average_stage_cost_three_oscillating_masses.pdf", format = 'pdf')
